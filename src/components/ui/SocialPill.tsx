...
import type { SocialIconName, SocialLink } from "@/lib/portfolio-data";
...
export function SocialPill({
  ...
  return (
    <a
      ...
      className={cn(
        "social-brand group inline-flex min-h-10 items-center justify-between gap-4 rounded-full border px-4 py-2 text-[12px] font-medium shadow-[0_9px_24px_rgba(0,0,0,.09)] backdrop-blur-xl transition-[transform,background-color,border-color,color,box-shadow] duration-500 ease-[var(--ease-apple)] hover:-translate-y-0.5 hover:text-white hover:shadow-[0_14px_34px_rgba(0,0,0,.16)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#111] focus-visible:ring-offset-2 sm:text-[13px]",
        inverted
        ...
        profile &&
          "min-w-[170px] border-black bg-[#1c1c1e] text-white shadow-[0_12px_32px_rgba(0,0,0,.2)] hover:border-black hover:bg-black hover:shadow-[0_16px_40px_rgba(0,0,0,.3)]",
        className,
      )}
    >
      <span
